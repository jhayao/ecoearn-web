...
ESP32_CAM_STREAM_URL = "http://192.168.123.49:81/stream"  # Update this with your ESP32-CAM IP
...
if __name__ == '__main__':
    ...
    print("ESP32-CAM Stream URL:", ESP32_CAM_STREAM_URL)
    print("\nAccess the monitoring interface at: http://localhost:5000/")
    print("API endpoints:")
    print("  - GET  /health - Health check")
    print("  - GET  /video_feed - Video stream with predictions")
    print("  - GET  /prediction - Latest prediction data")
    print("  - POST /identify/material - Identify material from image")
    
    # Start stream monitoring in background thread (even without model, for video display)
    monitor_thread = threading.Thread(target=monitor_stream, daemon=True)
    monitor_thread.start()
    if interpreter is not None:
        print("\n✓ Stream monitoring started - OpenCV window will display live feed with AI predictions")
    else:
        print("\n⚠️  Model not loaded - OpenCV window will display live feed WITHOUT AI predictions")
        print("   (Video stream will still work, but no material classification)")
