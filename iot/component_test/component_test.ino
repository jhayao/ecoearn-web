/*
 * EcoEarn Smart Bin - Component Test Sketch
 * 
 ...
 * - Serial Communication (ESP32-CAM ↔ ESP32)
...
const int PLASTIC_TRIG_PIN = 25;    // GPIO25
const int PLASTIC_ECHO_PIN = 26;    // GPIO26
const int TIN_TRIG_PIN = 27;        // GPIO27
const int TIN_ECHO_PIN = 14;        // GPIO14
...
// Sharp 2Y0A21 IR Sensor (ESP32 12-bit ADC: 0-4095)
const int USER_DETECTION_THRESHOLD = 1650;  // Scaled for 12-bit ADC
const int USER_DETECTION_MIN_ADC = 500;     // Scaled for 12-bit ADC
const int USER_DETECTION_MAX_ADC = 3500;    // Scaled for 12-bit ADC
...
void setup() {
  ...
  #if USE_GPS
    gpsSerial.begin(9600, SERIAL_8N1, GPS_RX_PIN, GPS_TX_PIN);  // ESP32 HardwareSerial
...
void printBanner() {
  ...
  Serial.println("║        Hardware Verification Tool         ║");
  Serial.println("╚════════════════════════════════════════════╝");
  Serial.println();
}
