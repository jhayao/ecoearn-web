...
const int TRASH_DETECTION_THRESHOLD = 1650;  // ADC threshold for trash detection (ESP32 12-bit ADC: 0-4095)
...
bool monitoringForTrash = false;      // NEW: ESP32 focuses on trash detection
...
void setup() {
  ...
  // Test ESP32-CAM communication with ping-pong
  Serial.println("╔════════════════════════════════════════╗");
  Serial.println("║   TESTING ESP32-CAM COMMUNICATION    ║");
  Serial.println("╚════════════════════════════════════════╝");
  bool camConnected = testESP32CAMConnection();
  
  if (camConnected) {
    Serial.println("✓ ESP32-CAM communication successful!");
    Serial.println("LED will stay ON for 3 seconds...\n");
    digitalWrite(LED_PIN, HIGH);
    delay(3000);
    digitalWrite(LED_PIN, LOW);
  } else {
    Serial.println("✗ ESP32-CAM communication FAILED!");
...
bool testESP32CAMConnection() {
  Serial.println("Sending PING to ESP32-CAM...");
  ...
  Serial.println("✗ No response from ESP32-CAM (timeout)");
  return false;
}
...
void openPlasticCompartment() {
  ...
  Serial.println("💡 Lid stays OPEN - user can throw more trash");
  Serial.println("🔄 Rotator returned to default position\n");
  ...
  ESP32_CAM_SERIAL.println("PLASTIC_OPENED");
}
...
void openTinCompartment() {
  ...
  ESP32_CAM_SERIAL.println("TIN_OPENED");
}
...
void openRejectedCompartment() {
  ...
  ESP32_CAM_SERIAL.println("REJECTED_OPENED");
}
